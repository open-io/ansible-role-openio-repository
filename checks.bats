...
@test 'Check repository files for multiple product distribution' {
  if [ "${DISTRIBUTION}" == "centos" ]; then
    ...
    if [ "${VERSION}" == '16.04' ]; then
      ...
      echo "status: "$status
      [[ "${status}" -eq "0" ]]
      [[ "${output}" =~ "/etc/apt/sources.list.d/openio-sds-16.10.list /etc/apt/sources.list.d/openio-sds-17.04.list /etc/apt/sources.list.d/openio-sds-18.04.list /etc/apt/sources.list.d/openio-test-18.04.list" ]]
    elif [ "${VERSION}" == '18.04' ]; then
      run docker exec -ti ${SUT_ID} bash -c 'ls /etc/apt/sources.list.d/openio* | tr "\n" " "'
      echo "output: "$output
      echo "status: "$status
      [[ "${status}" -eq "0" ]]
      [[ "${output}" =~ "/etc/apt/sources.list.d/openio-sds-18.04.list /etc/apt/sources.list.d/openio-test-18.04.list" ]]
    fi
  fi
}
